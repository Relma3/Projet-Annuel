...
<div class="max-w-3xl mx-auto mt-10 px-4">
    ...
    <p class="text-gray-500 mb-6">Echangez avec l'équipe Silver Happy</p>
    ...
    <div class="bg-white rounded-2xl shadow p-6 mb-6 space-y-4 min-h-64">
        ...
        <?php } else { ?>
            <?php foreach ($messages as $m) { ?>
                ...
                <div class="flex <?php if ($moi) { echo 'justify-end'; } else { echo 'justify-start'; } ?>">
                    <div class="max-w-xs lg:max-w-md px-5 py-3 rounded-2xl text-lg <?php if ($moi) { echo 'bg-orange-500 text-white'; } else { echo 'bg-gray-100 text-gray-800'; } ?>">

                        <?php if (!$moi) { ?>
                            <p class="text-sm font-semibold text-orange-500 mb-1">Equipe Silver Happy</p>
                        <?php } ?>

                        <p><?php echo htmlspecialchars($m['contenu']); ?></p>

                        <p class="text-xs mt-1 <?php if ($moi) { echo 'text-orange-100'; } else { echo 'text-gray-400'; } ?>">
    ...
    <form action="traitement_message.php" method="POST" class="bg-white rounded-2xl shadow p-6">
        ...
        <textarea name="contenu" rows="3" required
            ...
            class="w-full border-2 border-gray-200 rounded-xl p-4 text-lg focus:border-orange-400 focus:outline-none"></textarea>

        <button type="submit"
            class="mt-4 w-full bg-orange-500 text-white text-xl font-bold py-4 rounded-xl">
            Envoyer le message
        </button>
    </form>
</div>
