    ...
    <main class="pt-32 pb-20 px-6 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <aside class="lg:col-span-1">
            <div class="bg-white rounded-senior p-6 shadow-sm sticky top-28 space-y-2 border border-slate-100">
                <button onclick="showTab('actu', this)" class="tab-btn nav-active w-full flex items-center gap-4 p-4 rounded-2xl text-left font-bold transition-all">
                    <i class="fa-solid fa-house"></i> Vue d'ensemble
                </button>

                <button onclick="showTab('planning', this)" class="tab-btn w-full flex items-center gap-4 p-4 rounded-2xl text-left font-bold text-slate-400 hover:bg-peche-pale hover:text-corail transition-all">
                    <i class="fa-solid fa-calendar-check"></i> Mes RDV
                </button>

                <button onclick="showTab('commandes', this)" class="tab-btn w-full flex items-center gap-4 p-4 rounded-2xl text-left font-bold text-slate-400 hover:bg-peche-pale hover:text-corail transition-all">
                    <i class="fa-solid fa-box"></i> Mes Achats
                </button>

                <button onclick="showTab('messages', this)" class="tab-btn w-full flex items-center gap-4 p-4 rounded-2xl text-left font-bold text-slate-400 hover:bg-peche-pale hover:text-corail transition-all">
                ...
                <button onclick="showTab('profil', this)" class="tab-btn w-full flex items-center gap-4 p-4 rounded-2xl text-left font-bold text-slate-400 hover:bg-peche-pale hover:text-corail transition-all">
        ...
        <section class="lg:col-span-3 space-y-6">
            ...
            <div id="planning" class="tab-content space-y-6">
                    ...
                    <table class="w-full text-left">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <?php if (empty($planning)): ?>
                                <tr>
                                    <td colspan="3" class="p-6 text-center text-slate-300 italic">
                                        Aucun service réservé
                                    </td>
                                </tr>
                            <?php else: ?>
                                <?php foreach ($planning as $p): ?>
                                    <tr>
                                        <td class="p-6 font-bold">
                                            <?php echo date('d/m/Y à H:i', strtotime($p['date_reservation'])); ?>
                                        </td>
                                        <td class="p-6">
                                            <?php echo htmlspecialchars($p['p_prenom'] . ' ' . $p['p_nom']); ?>
                                        </td>
                                        <td class="p-6">
                    ...
                    <table class="w-full text-left">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <?php if (empty($rdv_medicaux)): ?>
                                <tr>
                                    <td colspan="4" class="p-6 text-center text-slate-300 italic">
                                        Aucun RDV médical planifié
                                    </td>
                                </tr>
                            <?php else: ?>
                                <?php foreach ($rdv_medicaux as $r): ?>
                                    <tr>
                                        <td class="p-6 font-bold">
                                            <?php echo date('d/m/Y à H:i', strtotime($r['date_rdv'])); ?>
                                        </td>
                                        <td class="p-6">
                                            Dr <?php echo htmlspecialchars($r['medecin_prenom'] . ' ' . $r['medecin_nom']); ?>
                                        </td>
                                        <td class="p-6 italic text-slate-400">
                                            <?php echo htmlspecialchars($r['specialite'] ?? '—'); ?>
                                        </td>
                                        <td class="p-6">
                                            <button onclick="annulerRdv(<?php echo (int) $r['id_rdv']; ?>)" class="text-xs text-red-400 hover:text-red-600 font-bold">
                                                Annuler
                                            </button>
                                        </td>
                                    </tr>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <?php else: ?>
                                <?php foreach ($evenements_inscrits as $e): ?>
                                    <tr>
                                        <td class="p-6 font-bold">
                                            <?php echo date('d/m/Y à H:i', strtotime($e['date_event'])); ?>
                                        </td>
                                        <td class="p-6">
                ...
                <div class="bg-white p-8 rounded-senior shadow-sm space-y-4">
                        ...
                        <div class="space-y-2">
                            ...
                            <select id="select-medecin" class="w-full bg-peche-pale/50 p-4 rounded-2xl font-bold text-corail outline-none">
                                ...
                                <?php foreach ($medecins as $m): ?>
                                    <option value="<?php echo (int) $m['id_prestataire']; ?>">
                    ...
                    <button onclick="prendreRdv()" class="bg-corail text-white px-10 py-4 rounded-2xl font-bold shadow-xl hover:scale-105 transition-transform">
            ...
            <div id="commandes" class="tab-content space-y-6">
                ...
                <div class="bg-white rounded-senior shadow-sm overflow-hidden text-sm">
                    <?php if (empty($commandes)): ?>
                        <p class="p-10 text-center text-slate-300 italic">
                                ...
                                <?php foreach ($commandes as $c): ?>
                                    <tr>
                                        <td class="p-6 font-medium">
                                            <?php echo htmlspecialchars($c['nom_article']); ?>
                                        </td>
                                        <td class="p-6 font-bold">
                                            <?php echo number_format($c['prix'], 2); ?> €
                                        </td>
                                        <td class="p-6 italic">
                    ...
                    <div class="flex justify-between items-center">
                        <div>
                            ...
                            <p class="text-emerald-500 font-bold mt-1">
                                <i class="fa-solid fa-check"></i> Adhésion 2026 active
                            </p>
                        </div>
                            ...
                            <div class="bg-white p-8 rounded-senior shadow-sm border-l-8 border-corail">
                                ...
                                <?php if ($c['auteur']): ?>
                                    <p class="text-xs text-slate-400 mt-4 italic">
                                        Par <?php echo htmlspecialchars($c['auteur']); ?>
                                    </p>
                                <?php endif; ?>
                            </div>
                        <?php endforeach; ?>
