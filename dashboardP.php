...
<body class="bg-[#F0F9F4] font-sans text-slate-800 min-h-screen">
    ...
    <nav class="fixed w-full bg-white/95 backdrop-blur-md shadow-sm z-50 px-8 py-4 flex justify-between items-center border-b border-emerald-100">
        <div class="flex items-center gap-10">
            ...
            <div class="hidden md:flex gap-8 text-sm font-bold text-slate-500 uppercase">
                <a href="index.php" class="hover:text-emerald-pro transition-colors">
                    Accueil
                </a>
            </div>
        </div>
        ...
        <section class="lg:col-span-3 space-y-6">
            ...
            <div id="services" class="tab-content space-y-6">
                <div class="flex justify-between items-center">
                    <h2 class="text-2xl font-title font-bold text-emerald-800">
                        ...
                        <?php foreach ($mes_services as $srv): ?>
                            <div class="bg-white p-6 rounded-senior shadow-sm border border-emerald-50 relative group">
                                <h3 class="font-bold text-lg text-emerald-700">
                                    <?php echo htmlspecialchars($srv['nom_service']); ?>
                                </h3>
            ...
            <div id="planning" class="tab-content space-y-6">
                <div class="bg-white p-8 rounded-senior shadow-sm border border-emerald-50">
                    <h2 class="text-2xl font-title font-bold text-emerald-800 mb-6">
                        Réservations reçues
                    </h2>
                            ...
                            <div class="flex justify-between items-center p-5 border border-slate-100 rounded-2xl bg-slate-50 mb-3">
                                ...
                                <div class="flex gap-2 items-center">
                                    ...
                                    <?php if ($r['statut'] === 'en_attente'): ?>
                                        <a href="act_res.php?id=<?php echo (int) $r['id_reservation']; ?>&a=accepter" class="bg-emerald-500 text-white text-xs px-3 py-1.5 rounded-lg font-bold hover:bg-emerald-600">
                                            ✓ Accepter
                                        </a>

                                        <a href="act_res.php?id=<?php echo (int) $r['id_reservation']; ?>&a=refuser" class="bg-red-100 text-red-500 text-xs px-3 py-1.5 rounded-lg font-bold hover:bg-red-200">
                                            ✗ Refuser
                                        </a>
                                    <?php elseif ($r['statut'] === 'confirme'): ?>
                                        <a href="act_res.php?id=<?php echo (int) $r['id_reservation']; ?>&a=terminer" class="bg-slate-200 text-slate-600 text-xs px-3 py-1.5 rounded-lg font-bold hover:bg-slate-300">
                                            Marquer terminé
                                        </a>
                                    <?php endif; ?>
                                </div>
            ...
            <div id="profil" class="tab-content space-y-6">
                <div class="bg-white p-8 rounded-senior shadow-sm border border-emerald-50">
                    ...
                    <div class="space-y-3 text-slate-600">
                        <p>
                            <strong>Nom :</strong>
                            <?php echo htmlspecialchars($nom_pres); ?>
                        </p>

                        <p>
                            <strong>Catégorie :</strong>
                            <?php echo htmlspecialchars($categorie_pres); ?>
                        </p>
                        ...
                        <?php if (!empty($profil['specialite'])): ?>
                            <p>
                                ...
                                <?php echo htmlspecialchars($profil['specialite']); ?>
                            </p>
                        <?php endif; ?>
                    </div>
                </div>
    ...
    <div id="modalService" class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[60] hidden flex items-center justify-center p-6">
        <div class="bg-white w-full max-w-md rounded-senior shadow-2xl overflow-hidden">
            <div class="bg-emerald-600 p-6 text-white flex justify-between items-center font-bold">
                ...
                <button onclick="toggleModal('modalService')" class="text-2xl">
                    &times;
                </button>
            </div>
            ...
            <form action="add_service.php" method="POST" class="p-8 space-y-4">
                <input 
                    type="text" 
                    name="nom_service" 
                    value="<?php echo htmlspecialchars($categorie_pres); ?>" 
                    readonly 
                    class="w-full p-4 bg-slate-100 border-none rounded-2xl text-slate-500 font-bold cursor-not-allowed outline-none"
                >
                ...
                <input 
                    ...
                    required 
                    class="w-full p-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-emerald-500 outline-none"
                >
                ...
                <textarea 
                    name="description" 
                    placeholder="Description courte (10 caractères min.)" 
                    rows="3" 
                    class="w-full p-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-emerald-500 outline-none"
                ></textarea>

                <button type="submit" class="w-full bg-emerald-600 text-white py-4 rounded-2xl font-bold shadow-lg hover:bg-emerald-700 transition-all">
                    Publier l'offre
                </button>
            </form>
        </div>
    ...
    <script>
                ...
                container.innerHTML = data.length === 0
                    ...
                    : data.map(dispo => `
                        <div class="flex justify-between items-center p-4 bg-slate-50 rounded-xl border border-slate-100">
                            <span>
                                Du ${new Date(dispo.date_debut).toLocaleString('fr-FR')}
                                au ${new Date(dispo.date_fin).toLocaleString('fr-FR')}
                            </span>

                            <button onclick="supprimerDispo(${dispo.id_disponibilite})" class="text-red-400">
        ...
        async function supprimerDispo(id) {
            ...
            await fetch(`api/dispos.php?id=${id}`, {
                method: 'DELETE'
            });
